...
import sys
import os
from util.conversion_module import F
...
def __true_to_mean(T,e):
    ...
    M = M%360
    return M
f=open('../../ADCS-Codes/sim_state.txt','w')
sys.stdout=f
...
def kep_to_sat(kep,epoch,bstar=0.21109E-4,whichconst=wgs72,afspc_mode=False):
    ...
    satrec.no_kozai= 86400/(2*np.pi*(kep[0]**3/398600.4405)**0.5)

    #temp = satrec.no()
    #temp = 86400/(2*np.pi*(kep[0]**3/398600.4405)**0.5)
    ...
    sgp4init(whichconst, afspc_mode, satrec.satnum, satrec.jdsatepoch-2433281.5, satrec.bstar, satrec.ndot, satrec.nddot, satrec.ecco, satrec.argpo, satrec.inclo, satrec.mo, satrec.no_kozai , satrec.nodeo, satrec)
