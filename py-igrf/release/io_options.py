...
def option1():
    ...
    '''
    # in_file = open('lla_pos.txt','r')
    # for iter in in_file.readline().split():
    #     lines.append(float(iter))
    # arr = lines[i].split()
    # i = i+1
    global a
    arr=[]
...
def write1(name, date, alt, lat, colat, lon, X, Y, Z, dX, dY, dZ, \
    ...
    #     with open(name, 'w') as file: 
            ...
            # field.append('{: 8.1f}'.format(X)+'\t')
            # field.append('{: 8.1f}'.format(Y)+'\t')
            # field.append('{: 8.1f}'.format(Z)+'\n')
